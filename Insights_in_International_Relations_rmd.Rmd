...
$\begin{align*} odds(Y_{ij} = 1) &= exp(\theta^Ts(\Delta Y_{ij}))\\ logodds(Y_{ij} = 1) &= \theta^Ts(\Delta Y_{ij})\\ logit(Y_{ij} = 1) &= \theta_1s_1(\Delta Y_{ij}) + \theta_2s_2(\Delta Y_{ij}) + \theta_3s_3(\Delta Y_{ij}) +.......\theta_ks_k(\Delta Y_{ij}) \end{align*}$
...
The mutual term, which measures reciprocity in edges, has a very low coefficient of $-13.853$ which tells us reciprocity was very rarely observed. 

The following terms have significant coefficients : Alliance, Border and Aid. However, all these coefficients are very high in magnitude which makes interpretation challenging.\
For example, according to results from our model, on the margin, having an alliance reduces the odds of a threat event occuring by about $1 - e^{-13.264} \approx 99.99983\%$! This might be due to occurence of very few threat events, almost none of which were between allies. This is the case for "presence of a border" and "Economic Aid" too.

Since the probability to logodds transformation is monotonic, a negative coefficient of $-12.219$ on "Aid" can be said to imply that providing economic aid to a country reduces the probability that it will threaten you (it reduces the odds of a threat event by $1 - e^{-12.219} \approx 99.99951\%$), which is consistent with our main hypothesis.

However, as is apparent, these estimates might just be a result of a really sparse network with too few threat events.
...
Datasets used : HDI dataset, UNDP
...
The Pearson's Chi-squared test used here is a parametric test whereas the Permutation Test in non-parametric. The Chi-squared test is an "approximate test". As sample size $n$ increases, the probability distribution of the test statistic $\sum_k \frac{(observed-expected)^2}{expected}$ converges to the Chi-squared distribution with $k-1$ degrees of freedom. However, when sample sizes are fairly small for some categories, as in our case, then the result might not be exact.
...
After many attempts, we narrowed down the dataset to just 24 actions. We filtered the actions on the basis of relevance (to the purpose of the index) and removed actions with too low variance.
...
p<-ggplot(data=Belligerance.index_high, aes(x=reorder(Country, -Index), y=Index)) +
  ...
  theme_minimal()
p1 <- p + theme_economist() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(title = "World Belligerent Index", x = "Country") + geom_hline(aes(yintercept=1), col="red") 
