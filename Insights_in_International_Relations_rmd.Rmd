...
We convert action counts to percentage of total actions by each country. This is done to avoid capturing variance in total activity amongst the countries.
```{r,eval=FALSE}
...
head(PCA_data_5[,1:7])
```
We use Eigenvalue decomposition to compute the Principal components. We create the symmetric covariance matrix and compute its Eigenvectors. These form the orthogonal eigenbasis of out covariance matrix by the Spectral Theorem.
...
These eigenvectors are our Principal Components. By the Spectral Theorem, we know they are orthogonal. We generate a Scree Plot and retain only the ones that explain more than 5% of total variance.
...
In this section, we use Linear Regression to model how our Index relates to some of the variables we computed and used in all our topics upto now.
...
where, $\hat{y}$ is the vector of projected y values and A is the matrix with $\mathbf{1}$ and the covariate vectors as its columns.
...
```
We find that the built-in R function lm() returns the same results. We use it to find the t-statistics, f-statistic and R-squared of our model.
```{r,comment=""}
...
* Appropriate use of covariance or correlation. (Topic 5, PCA uses a covariance matrix)
