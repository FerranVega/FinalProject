...
eventtype <- "<EEAI>" #Choose only economic aid events.
timeframe <- "weeks"
...
#Total for Aid
aid_totalvec <- c(vec1,vec95,vec90)

###############################################################

eventtype <- "<THRT>" #Choose only threat events.
timeframe <- "weeks"
...
#Total for Threats.
threat_totalvec <- c(vec1,vec95,vec90)
...
hist(aid_totalvec, probability = TRUE)
curve(dexp(x,l1$estimate),add = TRUE, type = "l")

hist(threat_totalvec, probability = TRUE)
curve(dexp(x,l2$estimate),add = TRUE, type = "l")

par(mfrow=c(1,1))

################# ADD CHI SQ. GOF TEST TO AID AND THREAT TOTALVEC.
################# CONVERT THE CODE TO A NICE LOOKING FUNCTION THAT SPITS OUT THE TOTALVEC
################# WHEN GIVEN AN EVENTTYPE AND TIMEFRAME.
################# IMPROVE GRAPHS USING GGPLOT2. ADD QQ OR PP.
################# NEXT SECTION WILL USE CHI-SQ. AND PERM TESTS ON AID-THREAT HYPOTHESES.
